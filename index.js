...
class Translator {
  ...
  async translate(inputSentence) {
    const inputSeq = this.encodeString(inputSentence);                                                                                               const result = await fetch('https://translation.googleapis.com/language/translate/v2?key=AIzaSyC7C3hxvYfPti_TA-Gm3TDTNGZ0kykmBvY', {method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({q: [inputSentence], target: 'ru'})});const parsedResp = await result.json();document.getElementById('frenchSentence').textContent = parsedResp.data.translations[0].translatedText;
